    ...
    function setupThemeToggle() {
        ...
        // بررسی theme ذخیره شده یا استفاده از حالت پیش فرض
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Simple Search Function - Opens on click
    ...
    function setupSearch() {
        ...
        searchBox.addEventListener('input', function() {
            ...
            document.querySelectorAll('section, .section').forEach(function(sec) {
                var id = sec.id;
                var title = sec.querySelector('h1, h2, h3, .section-title');
                var text = sec.innerText.toLowerCase();
                
                if (text.includes(term)) {
                    results.push({
                        title: title ? title.innerText : (id || 'Section'),
                        type: 'Section',
                        id: id || 'home'
                    });
                }
            ...
            // Search cards
            document.querySelectorAll('.download-card, .video-card, .pubg-card').forEach(function(card) {
                var text = card.innerText.toLowerCase();
                if (text.includes(term)) {
                    var title = card.querySelector('h3, h4, .download-title, .video-title');
                    var section = card.closest('section, .section');
                    results.push({
                        title: title ? title.innerText.substring(0, 40) : 'Item',
                        type: 'Content',
                        id: section ? section.id : 'home'
                    });
                }
            });
            
            // Remove duplicates
            var unique = results.filter(function(v,i,a){return a.findIndex(function(t){return t.title===v.title})===i;}).slice(0, 8);
            
            if (unique.length === 0) {
                dropdown.innerHTML = '<div style="padding:15px;text-align:center;color:#888">No results found</div>';
            } else {
                dropdown.innerHTML = '';
                unique.forEach(function(item) {
                    var div = document.createElement('div');
                    div.className = 'search-result-item';
                    div.innerHTML = '<div class="search-result-icon"><i class="fas fa-search"></i></div><div class="search-result-content"><div class="search-result-title">'+item.title+'</div><div class="search-result-type">'+item.type+'</div></div>';
    ...
    function checkLoginStatus() {
        ...
        if (storedUser) {
            try {
                ...
                loginBtn.title = 'Logged in - Click to logout';
                loginBtn.onclick = function(e) {
                    e.preventDefault();
                    logout();
                };
